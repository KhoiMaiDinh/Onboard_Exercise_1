...
| **`onboarding-exercise-1-shared`** | Contains shared libraries, domain models, and common utilities used across all modules.                     |
...
├── onboarding-exercise-1-shared/                       # Shared libraries 
├── docker.env                   # Docker environment variables for builds
├── k8s.env                      # Kubernetes manifests (deployments, services, etc.)
...
The `k8s.env/` directory contains all necessary Kubernetes manifests (Deployments, Services, ConfigMaps, Secrets) to run the system.
